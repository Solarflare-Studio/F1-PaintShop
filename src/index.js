...
// import {F1Fonts} from './f1Fonts' // maybe needed later if we expand languages to require additional fonts
...
// var f1Fonts = new F1Fonts(); // maybe required later for additional languages
...
//==================================================
let forcedsizeofcolourpicker = 0;
function createColourpicker() {
	
	// It's all sliders
	let forcedheightofcolourpicker = 0;//confirmButtonElement.offsetHeight * 1.;	

	// get what size we should generate the colour picker at
	// are we in a colour picker page
	const amincolourpickeralready = !document.getElementById('paintachannelblock').classList.contains("hidden");
	if(amincolourpickeralready) {
	...
	let keeppickcolour="rgb(255, 0, 0)";

	if(colorPatternPicker!=0) { // remove old
		keeppickcolour = colorPatternPicker.color.hexString;
...
function onConsole(_switch) {
	...
	if(_switch==0) {
		...
		specialPipelineToggle=!specialPipelineToggle;
	}
	else if(_switch==2) {
...
function postRenderProcess() {
	...
	if(doBuildBasemap) { 
		...
		// update cookie - one day we may need them
		var d = new Date();
...
function renderpipeline() {
	...
	if(specialPipelineToggle) {
		specialrenderpipeline();
		return;
	}
	// never here unless we're testing without any sfx
...
nextBtn.addEventListener("click", () => {
	...
	// bharti, this line below causes crash when building using webpack
